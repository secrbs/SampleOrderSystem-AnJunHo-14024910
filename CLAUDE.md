...
메인 메뉴 이외의 모든 서브 뷰는 상단에 사용자가 어떤 입력으로 현재 화면에 도달했는지 표시한다.
...
- 각 View의 `showMenu()` / `showForm()` 등은 `const std::string& path` 파라미터를 받아 상단에 출력
- path는 호출 측(main.cpp 또는 상위 run 함수)에서 구성해 전달
- 예: `"선택 > 2"`, `"선택 > 2 > 1"`
