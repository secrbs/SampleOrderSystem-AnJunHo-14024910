...
protected:
    void SetUp() override { writeJson(path, "[]"); }
...
TEST_F(Phase2Test, T2_01_RegisterValid) {
    ...
    EXPECT_TRUE(ctrl.registerSample({"S-001", "웨이퍼A", 0.5, 0.9, 0}));
...
TEST_F(Phase2Test, T2_02_RegisterDuplicateId) {
    ...
    EXPECT_FALSE(ctrl.registerSample({"S-001", "웨이퍼B", 0.3, 0.8, 0}));
    EXPECT_EQ(repo.count(), 1);
}
...
TEST_F(Phase2Test, T2_03_RegisterYieldZero) {
    SampleRepository repo(path);
    SampleController ctrl(repo);
    EXPECT_FALSE(ctrl.registerSample({"S-001", "웨이퍼A", 0.5, 0.0, 0}));
...
TEST_F(Phase2Test, T2_04_RegisterYieldOver1) {
    SampleRepository repo(path);
    SampleController ctrl(repo);
    EXPECT_FALSE(ctrl.registerSample({"S-001", "웨이퍼A", 0.5, 1.1, 0}));
    EXPECT_EQ(repo.count(), 0);
}

// T2-05: 시료 없는 상태 목록 조회
TEST_F(Phase2Test, T2_05_GetAllEmpty) {
    SampleRepository repo(path);
    SampleController ctrl(repo);
...
// T2-06: 등록 후 목록 조회
TEST_F(Phase2Test, T2_06_GetAllAfterRegister) {
    SampleRepository repo(path);
    SampleController ctrl(repo);
    ctrl.registerSample({"S-001", "웨이퍼A", 0.5, 0.9, 0});
    ctrl.registerSample({"S-002", "웨이퍼B", 0.3, 0.8, 0});
    EXPECT_EQ(ctrl.getAll().size(), 2u);
}
