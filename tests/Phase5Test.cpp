...
protected:
    void SetUp() override {
        ...
        writeJson(orderPath, R"([
            {"orderId":"ORD-001","sampleId":"S-001","customerName":"고객A","quantity":200,
             "status":"PRODUCING","createdAt":"","updatedAt":""},
            {"orderId":"ORD-002","sampleId":"S-001","customerName":"고객B","quantity":100,
             "status":"PRODUCING","createdAt":"","updatedAt":""},
            {"orderId":"ORD-003","sampleId":"S-001","customerName":"고객C","quantity":50,
             "status":"PRODUCING","createdAt":"","updatedAt":""}
        ])");
...
// T5-01: 실생산량 계산 (부족분 170, 수율 0.92)
TEST_F(Phase5Test, T5_01_CalcActualProduction) {
...
// T5-02: FIFO 순서 보장
TEST_F(Phase5Test, T5_02_FifoOrder) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    ProductionController ctrl(sr, or_);
...
// T5-03: 빈 큐 상태 조회
TEST_F(Phase5Test, T5_03_EmptyQueue) {
    writeJson(queuePath, "[]");
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    ProductionController ctrl(sr, or_);
...
// T5-04: 생산 완료 처리 → 재고 증가, CONFIRMED
TEST_F(Phase5Test, T5_04_CompleteProduction) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    ProductionController ctrl(sr, or_);
    ...
    // 재고 증가 (30 + 206 = 236)
    EXPECT_EQ(sr.findById("S-001")->stock, 236);
}

// T5-05: 생산 완료 후 다음 작업 자동 전환
TEST_F(Phase5Test, T5_05_NextJobAfterComplete) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    ProductionController ctrl(sr, or_);
...
// T5-06: 총생산시간 계산
TEST_F(Phase5Test, T5_06_CalcTotalTime) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    ProductionController ctrl(sr, or_);

    // avgTime 0.8, actualProduction 206
    EXPECT_DOUBLE_EQ(ctrl.calcTotalTime(0.8, 206), 164.8);
}

// T5-08: getCurrentJob — 큐에 항목 있을 때
TEST_F(Phase5Test, T5_08_GetCurrentJobNonEmpty) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    ProductionController ctrl(sr, or_);
...
// T5-07: 큐 영속성 — 재시작 후 복원
TEST_F(Phase5Test, T5_07_QueuePersistence) {
    {
        SampleRepository    sr(samplePath);
        OrderRepository     or_(orderPath, queuePath);
        ProductionController ctrl(sr, or_);
        ctrl.completeCurrentJob();  // ORD-001 완료
    }
