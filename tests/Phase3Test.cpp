...
TEST_F(Phase3Test, T3_02_InvalidSampleId) {
    SampleRepository sr(samplePath);
    OrderRepository  or_(orderPath, queuePath);
    OrderController  ctrl(sr, or_);

    EXPECT_FALSE(ctrl.placeOrder("S-999", "고객A", 10).has_value());
    EXPECT_EQ(or_.count(), 0);
}

// T3-03: 수량 0 이하 → nullopt
TEST_F(Phase3Test, T3_03_InvalidQuantity) {
    SampleRepository sr(samplePath);
    OrderRepository  or_(orderPath, queuePath);
    OrderController  ctrl(sr, or_);
