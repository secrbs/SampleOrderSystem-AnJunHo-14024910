...
protected:
    void SetUp() override {
        ...
        writeJson(orderPath, R"([
            {"orderId":"ORD-001","sampleId":"S-001","customerName":"A","quantity":50,"status":"RESERVED","createdAt":"","updatedAt":""},
            {"orderId":"ORD-002","sampleId":"S-001","customerName":"B","quantity":80,"status":"CONFIRMED","createdAt":"","updatedAt":""},
            {"orderId":"ORD-003","sampleId":"S-002","customerName":"C","quantity":30,"status":"PRODUCING","createdAt":"","updatedAt":""},
            {"orderId":"ORD-004","sampleId":"S-003","customerName":"D","quantity":50,"status":"CONFIRMED","createdAt":"","updatedAt":""},
            {"orderId":"ORD-005","sampleId":"S-001","customerName":"E","quantity":10,"status":"RELEASE","createdAt":"","updatedAt":""},
            {"orderId":"ORD-006","sampleId":"S-001","customerName":"F","quantity":5,"status":"REJECTED","createdAt":"","updatedAt":""}
        ])");
        writeJson(queuePath, "[]");
    }
...
// T6-01: 상태별 주문 수 집계 (REJECTED 제외)
TEST_F(Phase6Test, T6_01_OrderCountByStatus) {
...
// T6-02: REJECTED는 집계에 포함되지 않음
TEST_F(Phase6Test, T6_02_RejectedExcluded) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    MonitoringController ctrl(sr, or_);
...
// T6-03: 재고 0 → DEPLETED
TEST_F(Phase6Test, T6_03_StockDepleted) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    MonitoringController ctrl(sr, or_);
...
// T6-04: 재고 < CONFIRMED 합계 → SHORT
TEST_F(Phase6Test, T6_04_StockShort) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    MonitoringController ctrl(sr, or_);
...
// T6-05: 재고 ≥ CONFIRMED 합계 → SURPLUS
TEST_F(Phase6Test, T6_05_StockSurplus) {
    SampleRepository    sr(samplePath);
    OrderRepository     or_(orderPath, queuePath);
    MonitoringController ctrl(sr, or_);
