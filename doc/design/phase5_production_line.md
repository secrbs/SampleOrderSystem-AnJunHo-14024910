...
| ID | 내용 | 예상 결과 |
|----|------|-----------|
| T5-01 | 실생산량 계산 (부족분 170, 수율 0.92) | 206 |
| T5-02 | 큐 enqueue 3건 후 dequeue 순서 | FIFO 순서 보장 |
| T5-03 | 빈 큐 상태 조회 | 빈 큐 반환 |
| T5-04 | 생산 완료 처리 | 재고 증가, 상태 CONFIRMED |
| T5-05 | 생산 완료 후 다음 작업 자동 전환 | 큐 다음 항목이 current로 이동 |
| T5-06 | 총생산시간 계산 | avgTime × actualProduction |
| T5-07 | 큐 영속성: 재시작 후 큐 복원 | 동일 큐 상태 유지 |
| T5-08 | getCurrentJob — 큐에 항목 있을 때 | front 반환 |

## 완료 기준

- [x] ProductionLine 모델, FIFO 큐 구현
- [x] 실생산량 / 총생산시간 계산 로직
- [x] 생산 완료 시 재고 반영 및 상태 전환
- [x] 큐 JSON 영속성
